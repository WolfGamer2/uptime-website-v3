...
client.on("ready", () => {
  ...
  app.get('/api/v1', (req, res) => {
    res.json({ "GET": "api/v1/~", "api_status": "active", "api_version": "v1", "all_endpoints": "account", "checkAuth_endpoints": "account", "noCheckAuth_endpoints": "-", "total_endpoints": "1" })
  });
  ...
  app.get("/pick_eggs/countdown.:uuID", checkAuth, function(req, res) {
    var id = req.params.uuID
      ...
      renderTemplate(res, req, "pick_eggs/countdown.ejs", {id});
  });
  ...
  app.get("/monitors/view/:linkID", checkAuth, async (req, res) => {
    ...
    try {
      ...
      if (ly.fetch("linkler").filter(x => x.url === linkasil)[0].owner ===
        ...
        return renderTemplate(res, req, 'view.ejs', { url, link, linkasil })
    ...
    try {
      ...
      let url = ly.fetch("linkler").filter(x => x.pointID == linkasil)[0]
        ...
        return renderTemplate(res, req, 'statuspage_view.ejs', { url, link, linkasil })
